...
def create_argparser():
    ...
    parser.add_argument('--use_model', action='store_true', help="Use the model for prediction")
...
def main():
    ...
    with torch.no_grad():
       ...
       # Read in input file with reorientation
        ...
        I2 = torch.zeros([I.shape[0], np.ceil(I.shape[1] * av_thickness).astype(np.int32) , 
                          I.shape[2], 
                          I.shape[3]], device=device, dtype=dtype)
        
        I2linear, I2cubic = torch.zeros_like(I2), torch.zeros_like(I2)
        aff2 = aff.copy()
        ...
        for j in range(I2.shape[1]):
            ...
            # NEW: TRICUBIC INTERPOLATION
            ##################################################################################################################
            total_d = d1 + d2
            t = (d1 / total_d) if total_d > 0 else 0.0
            
            # Fetch 4 bounding neighbor indices (with clamping to prevent out-of-bounds crashes)
            idx0 = max(0, idx1 - 1)
            idx3 = min(I.shape[1] - 1, idx2 + 1)
            
            # Precalculate polynomial degrees
            t2 = t * t
            t3 = t2 * t
            
            # Catmull-Rom Spline Matrix Weights
            w0 = -0.5 * t3 + t2 - 0.5 * t
            w1 = 1.5 * t3 - 2.5 * t2 + 1.0
            w2 = -1.5 * t3 + 2.0 * t2 + 0.5 * t
            w3 = 0.5 * t3 - 0.5 * t
            
            # Calculate final cubic blend
            cubic_interp = (
                w0 * I[:, idx0, :, :] +
                w1 * I[:, idx1, :, :] +
                w2 * I[:, idx2, :, :] +
    ...
    I2cubic = (I2cubic.clip(0, 255).squeeze()).detach().cpu().numpy().astype(np.uint8)
    I2linear = (I2linear.clip(0, 255).squeeze()).detach().cpu().numpy().astype(np.uint8)
    output_file_nn = f'{args.save_path}'

    if '.mgz' in output_file_nn:
        output_file_nn = output_file_nn.replace('.mgz', '.nii.gz')

    MRIwrite(I2cubic, aff2, output_file_nn.replace('.nii.gz', '_tricubic.nii.gz'))
    I2cubic_gray = I2cubic.mean(axis=-1)
    MRIwrite(I2cubic_gray, aff2, output_file_nn.replace('.nii.gz', '_tricubic_gray.nii.gz'))
    
    MRIwrite(I2linear, aff2, output_file_nn.replace('.nii.gz', '_trilinear.nii.gz'))
    logger.log(f"Finished writing files")
