...
def _fmt_mean_sd(mean: float, sd: float, prec: int = CELL_PREC,
    ...
    when the SD is undefined (a single subject, so pandas std is NaN)."""
...
def build_error_latex(agg, labels):
    """Table 1: per-method error, reported as mean $\pm$ SD over subjects."""
    ...
    for si, sec in enumerate(present):
        ...
        for lab in labels:
            ...
            for method in METHODS:
                row = d[(d["Label"] == lab) & (d["Method"] == method)]
                if len(row):
                    cells.append(_fmt_mean_sd(float(row["norm_err_mean"].iloc[0]),
                                              float(row["norm_err_std"].iloc[0])))
                else:
                    cells.append("--")
...
def make_joint_figure(m_uw: pd.DataFrame, m_mad: pd.DataFrame) -> plt.Figure:
    ...
    for _, mm, _ in rows:
        if not mm.empty:
            ...
            pooled_x.extend((mm["Ref_mm3"].to_numpy()).tolist())
    if pooled_rel:
