...
def load_madrc(records: list) -> None:
    ...
    for method, pat in patterns.items():
        ...
        for f in glob.glob(os.path.join(MADRC_DIR, pat)):
            case = Path(f).parent.parent.name if method == 'Photo-recon' else Path(f).parent.name
...
def build_latex(df: pd.DataFrame) -> str:
    ...
    L.append(r"Region-specific Dice scores of automated segmentations of 3D "
             r"reconstructions of photographs before and after imputation (UNet-based and Tricubic interpolation-based), computed with Photo-Synthseg software." \
             r"Gold-standard "
             r"segmentations were obtained from MRI scans. }")
