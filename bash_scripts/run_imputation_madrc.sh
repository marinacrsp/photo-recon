...
OUTPUT_DIR=/home/marina/ms_thesis/photo_recon_madrc/03_bicubic_interpolations/
mkdir -p $OUTPUT_DIR
...
for subj_path in "$BASE_DIR"/*; do
    ...
    if [ -d "$subj_path" ]; then
        ...
        INPUT_FILE="${subj_path}/best_photo_recon/photo_recon.orig.mgz"
        ...
        if [ -f "$INPUT_FILE" ]; then
            echo " -> Sampling volume..."
            
            # Execute your python script with the mapped file parameters
            python "$PYTHON_SCRIPT" \
                --input_file "$INPUT_FILE" \
                --save_path "${OUTPUT_FILEPATH}/photo_recon.nii.gz"
                
            echo "    ✓ Saved to: $OUTPUT_FILE"
        else
            echo " -> [Skipping] Resolution file not found: photo_recon_tricubic.nii.gz"
        fi
    fi
done
