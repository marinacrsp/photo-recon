...
for subj_path in "$BASE_DIR"/*; do
    ...
    if [ -d "$subj_path" ]; then
        ...
        for res in "${RESOLUTIONS[@]}"; do
            INPUT_FILE="${subj_path}/photo_recon_tricubic_${res}.nii.gz"
            ...
            if [ -f "$INPUT_FILE" ]; then
                ...
                python "$PYTHON_SCRIPT" \
                    ...
                    --save_path "${OUTPUT_FILEPATH}/photo_recon_tricubic_${res}.nii.gz"
                    
                echo "    ✓ Saved to: $OUTPUT_FILE"
            else
                echo " -> [Skipping] Resolution file not found: photo_recon_tricubic_${res}.nii.gz"
            fi
        done
    fi
done
